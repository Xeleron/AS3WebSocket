						...
						if (frameQueue.length === 0)
						{
							if (frame.fin)
							{
								...
								if (messageCompressed)
								{
									try
									{
										inflatedData = WebSocketDeflate.decompress(binData);
										if (inflatedData.length > config.maxMessageSize)
										{
											drop(WebSocketCloseStatus.MESSAGE_TOO_LARGE, "Inflated message size exceeds maxMessageSize limit.");
											return;
										}
						...
						if (frameQueue.length === 0)
						{
							if (frame.fin)
							{
								...
								if (messageCompressed)
								{
									try
									{
										inflatedData = WebSocketDeflate.decompress(textPayload);
										if (inflatedData.length > config.maxMessageSize)
										{
											drop(WebSocketCloseStatus.MESSAGE_TOO_LARGE, "Inflated message size exceeds maxMessageSize limit.");
											return;
										}
										textPayload = inflatedData;
										if (debug)
										{
					...
					if (config.assembleFragments)
					{
						...
						if (frame.fin)
						{
							...
							if (fragCompressed)
							{
								try
								{
									inflatedData = WebSocketDeflate.decompress(binaryData);
									if (inflatedData.length > config.maxMessageSize)
									{
										drop(WebSocketCloseStatus.MESSAGE_TOO_LARGE, "Inflated message size exceeds maxMessageSize limit.");
										return;
				...
				while (lines.length > 0)
				{
					...
					else if (lcName === 'sec-websocket-extensions' && header.value)
					{
						...
						for (var ei:int = 0; ei < extensionsThisLine.length; ei++)
						{
							...
							{
								if (!deflateParams.serverNoContextTakeover)
								{
									failHandshake("Server declined 'server_no_context_takeover', which is strictly required by this client.");
									return;
								}
